...
**If you landed here directly** (not via `docs/EXECUTION_KIT.md`): also read
[docs/AGENT_CONSTITUTION.md](AGENT_CONSTITUTION.md) for the Constitutional Laws, Chief
foreman role, and the validated Build/Research approval-loop tables — this file and
that one are both real and current, covering different parts of the same system (see
AGENT_CONSTITUTION.md § Document Map for which wins where).
