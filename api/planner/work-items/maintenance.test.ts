...
import handler from "./maintenance.js";
...
describe("/api/planner/work-items/maintenance", () => {
  beforeEach(() => {
  ...
  it("filters to blocked work items", async () => {
    fetchPlannerWorkItemsByStatusMock.mockResolvedValue([{ id: "c" }]);
    ...
    expect(fetchPlannerWorkItemsByStatusMock).toHaveBeenCalledWith(["blocked"], 20);
    expect(res.body).toEqual({ workItems: [{ id: "c" }] });
  });
});
