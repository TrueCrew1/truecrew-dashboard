  ...
  if (events.length === 0) {
    return (
      <ApprovalSurfaceEmpty
        ...
        description="Session inspector for ADR-001 auditor events (observability only — not control). Proposal, decision, and reprioritization events appear here when you queue or decide on an Approvals card, or when the Board promotes an overdue task. They do not authorize merges, deploys, or agent work; logging failures do not block approvals."
      />
    );
  }
            ...
            <article className="chief-governance-card">
              ...
              <dl className="chief-governance-meta">
                <div className="chief-governance-meta-row">
                  <dt>{EVENT_ENTITY_LABEL[event.type]}</dt>
