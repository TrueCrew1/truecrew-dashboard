 ...
 * When open work is overdue across more than one workflow type, the current
 * sequencing isn't holding, which is exactly the Planner "Roadmap
 * reprioritization or re-sequencing" gate. Planner never asks the operator
 * directly: it builds a PlannerApprovalRequest and routes it through
 * createApprovalCardFromPlannerRequest() -> addCommandApproval() -> the shared
 * Chief queue, same as Build's real request.
 *
